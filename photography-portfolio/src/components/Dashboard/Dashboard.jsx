...
const Dashboard = () => {
	...
	const {
		...
		error: photosIdError,
	} = useFetchPhotosByIds(selectedCollection ? selectedCollection.photos : []);
			...
			<VStack width={"100%"} alignItems={"center"} justifyContent={"center"}>
				...
				{collectionsLoading ? (
					...
					>
											...
											<Box mb={4}>
												{categoriesFetched.map((category) => (
													<Button
														key={category.id}
														onClick={() =>
															handleCategoryClickModal(category.id)
														}
														colorScheme={
						...
						<Modal
							isOpen={isAddPhotosOpen}
							onClose={photosLoading ? undefined : onAddPhotosClose}
						>
							<ModalOverlay />
							<ModalContent>
								...
								<ModalFooter>
									<Button onClick={onAddPhotosClose} isDisabled={photosLoading}>
										Close
									</Button>
								</ModalFooter>
							</ModalContent>
